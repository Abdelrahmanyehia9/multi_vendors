...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    return BaseScaffold(
      appBar: BaseAppBar(actions: [_productAction()]),
      body: BaseBlocConsumer<ProductDetailsCubit, ProductDetailsModel>(
        successBuilder: (p) => ProductDetailsBody(model: p),
        loadingBuilder: () =>
            ProductDetailsBody(model: widget.product.toProductDetails()),
  ...
  Widget _productAction() {
    Widget action(ProductModel model) => Row(
      spacing: 4.w,
      children: [
        AppFavoriteButton(item: model, size: 20, padding: 8),
        // AppShareButton(
        //   onTap:()=> ShareService.instance.shareProduct(product: model),
        // ),
      ],
    );
